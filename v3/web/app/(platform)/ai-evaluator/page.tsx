  ...
  return (
    <main className="workspacePage">
      ...
      {runMethod === "local" && <section className="registrySection clientRunnerGuide" id="client-delivery">
        <header><div><p className="kicker">LOCAL AND CI RUNNER / ASSURANCE WORKFLOW</p><h2>Test the workflow customers actually use, without moving private data</h2><p>Run beside a model, RAG app, agent service, or multi-agent system. Start with one customer-facing workflow, not every internal agent. Discovery is local, scope is customer-confirmed, and results stay local unless your team later chooses a signed shared decision.</p></div></header>
        <div className="clientRunnerSteps">
          <div><span>01 / INSTALL THE VERIFIED RUNNER</span><strong>Only download the small runner</strong><a href="https://github.com/lucky1410/ExposureScopeX/releases" target="_blank" rel="noreferrer">OPEN GITHUB RELEASES</a><code>py -m pip install .\exposurescopex_eval_runner-&lt;version&gt;-py3-none-any.whl</code><small>Download the versioned wheel and <code>SHA256SUMS</code>, verify the checksum, then install. Do not download source code or the full platform. <a href="https://github.com/lucky1410/ExposureScopeX/blob/main/v3/client-runner/README.md" target="_blank" rel="noreferrer">OPEN THE FULL LOCAL GUIDE</a></small></div>
          <div><span>02 / DISCOVER, THEN CONFIRM</span><strong>Choose what is safe to test</strong><code>esx-eval discover --repository C:\work\my-app --out .\discovery.json</code><code>esx-eval scope --discovery .\discovery.json --include &lt;component-id&gt; --out .\scope.json</code><small>The runner identifies likely workflows, agent frameworks, tools, retrieval stores, and observability signals without exporting source or executing code. Review the IDs, then confirm only the components your team authorizes.</small></div>
          <div><span>03 / REVIEW A TEST PLAN</span><strong>Get the right coverage for the scope</strong><code>esx-eval plan --scope .\scope.json --out .\risk-plan.json</code><small>The plan selects classification, security, trajectory, RAG, robustness, repeatability, agreement, or cost checks only when the confirmed scope supports them. It is deterministic and reviewable: no hidden external AI call or automatic execution.</small></div>
          <div><span>04 / OPTIONAL EVIDENCE PATHS</span><strong>Use browser or telemetry when needed</strong><code>esx-eval telemetry --out .\out\telemetry.jsonl</code><code>py -m pip install &quot;exposurescopex-eval-runner[browser]&quot;</code><small>Existing OpenTelemetry JSON can be sent to a token-protected loopback collector that redacts prompts and outputs. Browser journeys are optional, loopback-only Playwright checks using approved test accounts. Neither path tests arbitrary remote systems.</small></div>
          <div><span>05 / RUN AND REVIEW LOCALLY</span><strong>See evidence, gaps, and results</strong><code>esx-eval run --config .\esx-eval.json --out .\out\evaluation.json --discovery .\discovery.json --scope .\scope.json --plan .\risk-plan.json</code><code>esx-eval view --report .\out\evaluation.local-report.html</code><small>The terminal shows case outcomes; the local report shows the Assurance Graph: discovered items, confirmed scope, measured metrics, missing evidence, and why no local result is automatically a release decision. Upload is optional and separate.</small></div>
        </div>
        <p className="clientRunnerBoundary"><strong>What this does not do:</strong> it does not silently discover and execute every agent, browser flow, or tool in an unknown application. That would be unsafe and misleading. The customer confirms scope and supplies only approved test access.</p>
      </section>}
