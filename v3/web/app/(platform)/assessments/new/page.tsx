...
const profileCopy = {
  light: ["Rapid exposure baseline", "Deterministic and non-exploitative", "Best for frequent triage and initial review"],
    ...
    <main className="workspacePage newAssessmentPage">
      ...
      <form className="assessmentBuilder" onSubmit={submit} ref={formRef}>
        <div className="builderMain">
          ...
          <section className="builderSection"><div className="sectionIndex">02</div><div className="sectionBody"><div className="sectionHeading"><h2>Target configuration</h2><p>Name the engagement and define the exact authorized target.</p></div><div className="fieldGrid"><label>Engagement name<input name="name" placeholder="External web assessment" required /></label><label>Target URL<input name="target" type="url" placeholder="https://example.com" required /></label></div></div></section>
          <section className="builderSection"><div className="sectionIndex">03</div><div className="sectionBody"><div className="sectionHeading"><h2>Assessment profile</h2><p>Profiles control coverage and resource budgets, not finding severity.</p></div><div className="profileGrid">{(Object.keys(profileCopy) as Array<keyof typeof profileCopy>).map((item) => <button className={`profileChoice ${mode === item ? "profileChoiceActive" : ""}`} type="button" key={item} onClick={() => { setMode(item); setPreview(null); }}><span>{item}</span>{profileCopy[item].map((line) => <small key={line}>{line}</small>)}</button>)}</div></div></section>
          <section className="builderSection"><div className="sectionIndex">04</div><div className="sectionBody"><div className="sectionHeading"><h2>Authenticated coverage</h2><p>Optionally establish a scoped test-user session. Credentials are encrypted and never written to evidence or logs.</p></div><label className="authorizationCard"><input type="checkbox" checked={authenticated} onChange={(event) => setAuthenticated(event.target.checked)} /><span><strong>Use authorized test credentials</strong><small>The login URL must use the same scheme, hostname, and port as the assessment target.</small></span></label>{authenticated && <div className="authFieldGrid"><label>Login URL<input name="login_url" type="url" required placeholder="https://app.example.com/login" /></label><label>Username<input name="auth_username" required autoComplete="off" /></label><label>Password<input name="auth_password" type="password" required autoComplete="new-password" /></label><label>Username selector<input name="username_selector" required defaultValue={'input[name="username"]'} /></label><label>Password selector<input name="password_selector" required defaultValue={'input[type="password"]'} /></label><label>Submit selector<input name="submit_selector" required defaultValue={'button[type="submit"], input[type="submit"]'} /></label></div>}</div></section>
          <section className="builderSection"><div className="sectionIndex">05</div><div className="sectionBody"><div className="sectionHeading"><h2>Authorization gate</h2><p>Execution remains blocked until authorization is explicitly confirmed.</p></div><label className="authorizationCard"><input name="authorization" type="checkbox" required /><span><strong>Written authorization confirmed</strong><small>I confirm authority to perform the selected non-exploitative assessment against this exact target.</small></span></label></div></section>
        </div>
