...
class SystemEvaluationTests(unittest.TestCase):
    ...
    def test_coverage_pack_drafts_and_evidence_gaps_are_review_only(self):
        with TemporaryDirectory() as directory:
            ...
            self.assertIn("FIX PLAN", rendered)
            self.assertIn("HARNESS ENGINEERING RECOMMENDATIONS", rendered)
            self.assertIn("What should the team fix next?", rendered)
    ...
    def test_failed_cleanup_stops_later_checks(self):
        with TemporaryDirectory() as directory, reference_app() as (url, state):
            ...
            self.assertIn("REPORT GUIDE", rendered)
            ...
            self.assertIn("Open metric evidence table", rendered)
            self.assertIn("Verified AI metrics", rendered)
            self.assertIn("Strictly complete components", rendered)
            self.assertIn("strict all-required-evidence count, not the execution count", rendered)
            self.assertIn("partial evidence is shown in the area/module sections", rendered)
            self.assertNotIn("Components with required checks executed", rendered)
            self.assertIn("COVERAGE MAP", rendered)
            self.assertIn("Which parts of the application had usable evidence?", rendered)
            self.assertIn("Open evaluation area table", rendered)
            self.assertIn("FIX PLAN", rendered)
            self.assertIn("HARNESS ENGINEERING RECOMMENDATIONS", rendered)
            self.assertIn("What should the team fix next?", rendered)
            self.assertIn("Open harness engineering recommendations", rendered)
            self.assertIn("FINDINGS AND PROOF", rendered)
            self.assertIn("Open finding preview", rendered)
