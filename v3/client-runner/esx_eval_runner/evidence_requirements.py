...
METRIC_REQUIREMENTS: dict[str, dict[str, str]] = {
    ...
    "groundedness": {
        "title": "Groundedness",
        "user_supplies": "A review policy for what counts as supported evidence for each answer claim.",
        "application_emits": "Per-claim opaque evidence IDs, support score, citation-valid flag, and evidence-integrity flag.",
        "minimum": "At least one claim with a cited, integrity-verified evidence reference and a local support assessment. Evidence-ID overlap alone is insufficient.",
...
_MEASUREMENT_FIELD = {"groundedness": "claims", **{name: name for name in _ADVANCED_DIMENSIONS if name != "groundedness"}}
_TELEMETRY_GAPS = {
    "groundedness": "Emit claim records with esx.claim_id, esx.evidence_id, esx.claim_support_score, esx.citation_valid, and esx.evidence_integrity_valid.",
...
def _advanced_measurement_status(
    ...
    from .local_metrics import (
        _agreement, claims_metrics, rag_metrics, robustness_metrics,
    ...
    metric = {
        "groundedness": lambda: claims_metrics(value),
...
def inspect_evidence_preflight(
    ...
    for metric in required:
        ...
        if sources:
            ...
            status, missing = _advanced_measurement_status(metric, raw, case_ids)
    ...
    return {
        ...
        "connection": {"adapter_type": adapter_type, "telemetry_path": str(telemetry_file) if telemetry_file else None, "measurements_path": str(measurements_file) if measurements_file else None},
...
def render_evidence_requirements_markdown(config: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "This plan-specific checklist states what PRE-D needs before it will calculate each requested score. All inputs and calculations remain local. Do not add raw prompts, answers, document text, credentials, cookies, or production data; use opaque local identifiers.",
        "",
        f"- **Connection:** `{connection}`",
        f"- **Requested metrics:** {', '.join(entry['title'] for entry in entries) or 'none'}",
        "",
        "A score is calculated only after PRE-D receives both the team-approved expectation and the matching observed local evidence. A missing item produces `EVIDENCE NEEDED`, never an estimated score.",
        "",
    ]
    for entry in entries:
        lines.extend([
