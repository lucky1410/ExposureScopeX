...
**PRE-D Local 0.15.3: source-first full-platform onboarding, hardened source protection, coverage advisories, reviewed module scope, historical context, and local AI evaluation.**
...
| See what changed and what remains unsupported | [0.15.3 release notes](RELEASE_NOTES.md) |
...
PRE-D Local 0.15.3 provides profile bootstrap/refresh, bounded planning
...
## What is new in 0.15.3

- Source-protection fingerprinting now excludes generated/vendor/build trees and
  common binary artifacts by default, while keeping configurable `scan_limits`
  in the protected profile. A byte/file limit hit is reported as an incomplete
  fingerprint, not as proof that application source changed.
- Protected profile runs reuse the same configured source scan limits during
  setup, preflight, execution and after-check verification, preventing a real
  repository from failing because setup and run used different assumptions.
- Browser session reuse validates the saved local session against the approved
  success signal before protected journeys run. Stale session-bootstrap/SSO
  profiles block at session setup with a reauthentication signal; form-auth
  profiles can refresh through approved environment variables.
- Scope binding errors now identify the mismatched objective/check component,
  layer, and role instead of returning a generic binding failure.
- System reports now explain terminal execution counts and show whether source
  integrity is changed, incomplete, or not configured.
...
your evaluation environment. The version should be `0.15.3`. If the import
...
3. Verify and install the downloaded wheel. Use the published runner version:
   ...
   python3 -m pip install --upgrade ./exposurescopex_eval_runner-0.15.3-py3-none-any.whl
