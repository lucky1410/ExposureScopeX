...
**PRE-D Local 0.12.0: local AI evaluation and application release review.**
...
| See what changed and what remains unsupported | [0.12.0 release notes](RELEASE_NOTES.md) |
...
your evaluation environment. The version should be `0.12.0`. If the import
shows the new version but `esx-eval` has no `release` command, the executable on
...
3. Verify and install the downloaded wheel. Use the published runner version:
   ...
   python3 -m pip install --upgrade ./exposurescopex_eval_runner-0.12.0-py3-none-any.whl
