...
**PRE-D Local 0.13.0: guided setup, reviewed module coverage, and local AI evaluation.**
...
| See what changed and what remains unsupported | [0.13.0 release notes](RELEASE_NOTES.md) |
...
- `esx-eval setup --application` provides local forms for module inventory,
  existing evaluation plans, personas, dependencies, and reviewed case bindings.
- `release scope`, `bind`, `attach`, `preflight`, and `run` prepare and execute
  the entire declared inventory. Missing plans or objectives block execution
  before a target is called; a completed report is not a fresh execution.
- Reusable objectives cover positive, negative, boundary, recovery,
  authorization, and integration scenarios. They are review templates, not
  generated tests or expected labels.
- Reports distinguish actual execution, objective outcomes, dependency gaps,
  observed decision errors, and selected release thresholds.
- Plan approval is tied to its configuration; guided creation requires the
  reviewed preview to match current plans and bindings. Local setup requests
  are serialized to avoid cross-request path resolution errors.
...
### Retained from 0.12.0

The new `release` commands build on the existing metric engines, rather than
...
| Semantic groundedness/hallucination | Claim-level judgments against supplied sources, plus supported abstention checks | Actual response text, exact source material, an independent local judge, and configured review/gold expectations |
...
**Available in 0.13.0: guided application setup and all-module execution.**
...
setup; unbound objectives remain visible and block all-module execution.
...
execution block this mode before any suite starts. Upgrade to 0.13.0 to use
...
your evaluation environment. The version should be `0.13.0`. If the import
...
current-release gates, but their regression comparisons are not yet supported.
Declared inventory is not automatic discovery of every business journey. The
release layer does not add load tests, arbitrary permissions probes, automatic
root-cause localization, or production-wide correctness guarantees. Local test
fixtures validate runner behavior, not independent real-application acceptance.
...
3. Verify and install the downloaded wheel. Use the published runner version:
   ...
   python3 -m pip install --upgrade ./exposurescopex_eval_runner-0.13.0-py3-none-any.whl
